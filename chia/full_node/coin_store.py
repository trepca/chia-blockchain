...
class CoinStore:
    ...
    def row_to_coin(self, row) -> Coin:
        if self.db_wrapper.db_version == 2:
            amount = struct.unpack(">Q", row[5])[0]
        else:
            amount = uint64.from_bytes(row[5])
        return Coin(bytes32(self.maybe_from_hex(row[4])), bytes32(self.maybe_from_hex(row[3])), amount)
            ...
            for record in records:
                ...
                values2.append(
                    (
                        ...
                        struct.pack(">Q", record.coin.amount),
                        record.timestamp,
                    )
                )
