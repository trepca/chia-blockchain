...
def make_bundle(
    latest_singleton, launcher_id, coinsol, solution, value, fee=0, fee_coin=None, puzzle=COUNTER_PUZZLE
) -> SpendBundle:
...
async def test_mempool_spend_aggregation(two_nodes_one_block, wallet_a, self_hostname) -> None:
    ...
    starting_coin = open_coins[1]
    fee_coins = open_coins[2:]
    ...
    first_bundle: SpendBundle = SpendBundle([singleton_eve_coinsol, starting_coinsol, launcher_coinsol], G2Element())
    first_bundle.debug()
    tx = wallet_protocol.SendTransaction(first_bundle)
    msg = await full_node_1.send_transaction(tx, test=True)
    msg = TransactionAck.from_bytes(msg.data)
    print(msg)
    assert msg.status == MempoolInclusionStatus.SUCCESS
    latest_singleton = [x for x in first_bundle.additions() if x.amount == 1][0]
    _ = await next_block(full_node_1, wallet_a, bt)

    bundle1 = make_bundle(latest_singleton, launcher_id, singleton_eve_coinsol, [1000], 1000)

    tx = wallet_protocol.SendTransaction(bundle1)
    msg = await full_node_1.send_transaction(tx, test=True)
    msg = TransactionAck.from_bytes(msg.data)
    assert msg.status == MempoolInclusionStatus.SUCCESS
    bundle2 = make_bundle(
    ...
    tx = wallet_protocol.SendTransaction(bundle2)
    msg = await full_node_1.send_transaction(tx, test=True)
    msg = TransactionAck.from_bytes(msg.data)
    assert msg.status == MempoolInclusionStatus.SUCCESS
